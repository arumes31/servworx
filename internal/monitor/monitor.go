...
func updateServiceStatus(serviceName string, success bool) {
	_ = config.UpdateStatus(func(s *config.Status) {
...
func handleServiceFailure(svc *config.ServiceConfig, lastRestart int64) int64 {
	now := time.Now().Unix()
	timeSinceLastRestart := now - lastRestart
	restartAllowed := timeSinceLastRestart >= int64(svc.GracePeriod)
	remainingGrace := int64(svc.GracePeriod) - timeSinceLastRestart
	if remainingGrace < 0 {
		remainingGrace = 0
	}
...
func monitorService(svc config.ServiceConfig) {
	...
	for {
		...
		success, message := checkWithRetries(currentSvc)
		if message == "Stopped" {
			return
		}

		if !success {
			lastRestart = handleServiceFailure(currentSvc, lastRestart)
		}
